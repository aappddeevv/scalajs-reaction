...
object Results {
  ...
  def make(cache: Cache, query: Option[String], onActiveResultUpdate: Option[Movie] => Unit,
          ...
          div(new DivProps{ className = cn.root })(
            arr.take(5).map(m =>
              div(new DivProps{ key = m.id.toString() })(
                Result.make(cache, m,
                  onActiveResultUpdate, activeResult.map(_.id == m.id).getOrElse(false)))))
...
object Result {
  ...
  def make(cache: Cache, result: Movie, onActiveResultUpdate: Option[Movie] => Unit, isActive: Boolean) =
    render { self =>
      val config = data.config(cache, configKey)
      ...
      button(new ButtonProps{
        ...
        div(new DivProps{ className = cn.content})(
          div(new DivProps { className = cn.thumbnail})(
            result.poster_path.toNonNullOption.map(p => PosterThumbnail.make(src=s"${baseURL}/$size/${p}")).getOrElse(null)),
  ...
  private def _getClassNames(w: Double, h: Double, isActive: Boolean = false) =
    FabricStyling.mergeStyleSets[ResultClassNames](
      styleset(
        "button" -> stylearray(new IRawStyle {
          ...
          if(self.state.error) "Error encountered, see dev console. Refresh browser tab."
            ...
            AsyncValue.make(self.state, defaultStateValue) { asyncState =>
              MasterDetail.make(
                ...
                results = Results.make(cache, self.state.query,
                  ar => self.send(ActiveResult(ar)), self.state.activeResult),
                details =
